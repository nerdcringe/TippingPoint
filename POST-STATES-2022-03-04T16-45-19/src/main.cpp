...
// winpoint, get center at end

/* // UPDATED AS OF 4:30 on 3-2
   // Edit statesSkills
   //	First number is distance/angle
   // Second number is speed

   // gyroTurn turns to the angle relative to the angle the robot starts
   // relativeTurn turns relative to the current angle

   // task::sleep waits for milliseconds, so 1 second is task::sleep(1000);
STATES:
 - Skills						ok
 - RightSide					yes
 - LeftSide						yes
...
void simpleForward(double inches, double speed)
{
  ...
  clearEncoders();
  LFBASE.startRotateFor(ticks, deg, speed, velocityUnits::pct);
  LMBASE.startRotateFor(ticks, deg, speed, velocityUnits::pct);
  LBBASE.startRotateFor(ticks, deg, speed, velocityUnits::pct);
  RFBASE.startRotateFor(ticks, deg, speed, velocityUnits::pct);
  RMBASE.startRotateFor(ticks, deg, speed, velocityUnits::pct);
  RBBASE.rotateFor(ticks, deg, speed, velocityUnits::pct);
  /*
  double error = ticks;
...
// Moves forward at least a certain distance but doesn't correct if it overshoots
// As far as I know it only works for fwd, not backward
void forwardInchesFast2(double inches, int maxSpeed, int msTimeout = -1, double lSpeedMult=1, int deaccelRate=4)
{
  clearEncoders();
  //maxSpeed = keepInRange(maxSpeed, 0, 100);
  ...
  (Brain.timer(vex::timeUnits::msec) < msTimeout || msTimeout == -1))
  {
    ...
    leftDrive(speed * lSpeedMult/* + correctionL*/);
    rightDrive(speed/* + correctionR*/);
...
void leftSide()
{
  ...
  rightDrive(-100);
  task::sleep(250);
  ARM.startRotateTo(75, deg, 80, velocityUnits::pct);
}
...
void rightSideRings()
{
  ...
  // bring close yellow mogo and drop it in zone
  forwardInchesFast(-18, 100); //-23 dist
  ...
  gyroTurn(-97, 32);   // -95 too right
  forwardInchesTimed(-6.5, 42, 1200); // into wall
  task::sleep(100);
  forkliftDown(); // forkliftDown means bite down, not place down

  forwardInches(3, 18); // line up with rings, 6.75, 6.25
  ARM.startRotateTo(200, deg, 50, velocityUnits::pct);
  gyroTurn(-1, 25);
  conveyorOn();
  forwardInches(25, 20);
...
void leftMatchLoads()
{
  ...
  rightDrive(-100);
  task::sleep(250);
  forwardInchesFast(-31, 100, 5000);
  /*leftDrive(-95);
  rightDrive(-100);
  task::sleep(3000);*/
  //simpleForward(-40, 100);
  forwardInches(1, 14);
  gyroTurn(-75 , 35);
  forwardInches(-12, 35);
  forkliftDown();
...
int dropMogoDelay()
{
  ...
  conveyorOff();*/
  while(ticksToInches(encoderAverage()) < 40) {
    task::sleep(5);
  }
  conveyorOff();
  task::sleep(250);
...
void fullRow()
{
  ...
  clearEncoders();
  task b(dropMogoDelay); // drop the mobile goal after some encoder value is reached while moving

  forwardInchesFast(60, 100); // go across home row

  //forkliftUp();
  gyroTurn(180, 30);
  forwardInchesFast(-28, 70, 1300);
  forwardInches(2, 22);
  gyroTurn(92, 30);
...
int lowerArmFullRowCenter()
{
  while(ticksToInches(encoderAverage()) < 20) {
    task::sleep(5);
  }
  ARM.startRotateTo(-50, deg, 65, velocityUnits::pct);
...
void fullRowCenter()
{
  ...
  // Pick up mogo
  forkliftUp();
  forwardInchesFast(-9, 34);
  forkliftDown();

  // Score rings on mogo
  task::sleep(150);
  forwardInchesFast(1.5, 17);
                        //forwardInches(-5, 25);
  gyroTurn(90, 30);
  conveyorOn();
  ARM.startRotateTo(890, deg, 50, velocityUnits::pct);
  ...
  clearEncoders();
  task b(dropMogoDelayCenter); // drop the mobile goal after some encoder value is reached while moving

  forwardInchesFast(60, 135); // go across home row

  //forkliftUp();
  
  gyroTurn(180, 30);
  forwardInchesFast(-28, 70, 1150);
  forwardInches(2, 22);
  gyroTurn(92, 30);

  forwardInchesSpeedUp(-14, 30); // make faster
  forkliftDown();

  forwardInches(10, 26);
  ARM.startRotateTo(200, deg, 50, velocityUnits::pct);
  gyroTurn(223, 33); // more left
  task c(lowerArmFullRowCenter);
  forwardInchesSpeedUp(35, 80);
  clampDown();
...
void statesSkills()
{
  // set the arm's current position to zero to make all movements relative to when it is fully down
  ARM.resetRotation();
  ...
  forwardInches(3, 16);
    task::sleep(100);
  gyroTurn(99.5, 32); // turn to neutral
  
  // BRING CLOSE NEUTRAL + LEFT RED -> RED SIDE 40 (+40)
  ...
  // SCORE CLOSE NEUTRAL -> RED PLATFORM: 60 (+20)
  ARM.startRotateTo(1050, deg, 85, velocityUnits::pct);
  task::sleep(175);
  forwardInches(28.5, 43); // go into platform
  ARM.rotateTo(700, deg, 65, velocityUnits::pct);

  gyroTurn(100, 20); // turn into platform to balance it
  ...
  // DROP LEFT RED
  forwardInches(-3.75, 24);
  gyroTurn(0, 27);
  ARM.startRotateTo(-75, deg, 80, velocityUnits::pct);
  forwardInches(5, 25); // back up to get mobile goal over rings
  forkliftUp();
  task::sleep(75);
  
  // GET RIGHT BLUE
  forwardInchesFast(18.5, 58);
  clampDown();
  //forkliftDown();
  task::sleep(75);
  //conveyorOn();

  // PUSH CENTER NEUTRAL + RIGHT BLUE -> BLUE ZONE: 100 (+40)
  forwardInches(-11.5, 38);
  gyroTurn(42.5, 23);
 
  //forwardInchesFast(-29, 60); 
  //forkliftDown();
  //forwardInchesFast(-34, 70);
  forwardInchesFast(-64, 135); // push tall neutral into blue zone
  forkliftUp();
  task::sleep(75);
  ...
  task::sleep(50);
  ARM.startRotateTo(-50, deg, 60, velocityUnits::pct);
  forwardInchesTimed(-26, 35, 2150); // back into mobile goal against wall
  forkliftDown();
  task::sleep(50);
  conveyorOn();
  gyroTurn(54, 29);

  // BRING FAR NEUTRAL + RIGHT RED -> RED SIDE: 160 (+40)
  ARM.startRotateTo(150, deg, 50, velocityUnits::pct); // lift arm so conveyor can intake rings
  forwardInches(17, 37);
  ARM.startRotateTo(-50, deg, 50, velocityUnits::pct); // lower arm to pick up mobile goal
  task::sleep(200);
  forwardInches(6, 24);
  clampDown();
  task::sleep(25);
  ...
  // SCORE FAR NEUTRAL -> RED PLATFORM: 180 (+20)
  ARM.startRotateTo(850, deg, 80, velocityUnits::pct);
  task::sleep(300);
  forwardInches(25, 32);
  clampUp();
 // forwardInches(-2, 18);
 ...
 // PICK UP LEFT RED
  forwardInches(-5, 22);
  gyroTurn(-3, 29);
  // get rings in front of mobile goal
  //ARM.rotateTo(350, deg, 75, velocityUnits::pct);
  //forwardInches(7, 23);
  //forwardInches(-7, 23);
  ARM.rotateTo(-35, deg, 75, velocityUnits::pct);
  forwardInches(12, 35);
  clampDown();
  task::sleep(100);

  // SCORE LEFT RED -> RED PLATFORM: 200 (+20)
...
*/
  ...
  // SCORE LEFT BLUE -> BLUE PLATFORM: 240 (+40)
  gyroTurn(-70, 33);
  ARM.startRotateTo(800, deg, 75, velocityUnits::pct);
  forwardInchesFast(50, 50);
  clampUp();
  */
}
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    */
    ...
    Brain.Screen.printAt(220, 170, "Encoder ticks: %.2f deg  ", encoderAverage());
...
int main() {
  ...
  // fairgroundsSkills
  Competition.autonomous(leftMatchLoads);

...
// no time to correct
void smoothSpeedFast(double inches, double startSpeed, double endSpeed)
{
  double ticks = inchesToTicks(inches);
  clearEncoders();
  double error = ticks;
...
void forwardInches(double inches, int maxSpeed, double minSpeed = 2)
{
  clearEncoders();
  ...
  const double deaccelRate = 12;  // Speed multiplier while slowing down (slope at end). Starts slowing down later when higher
...
void forwardWall(double inches, int maxSpeed)
{
  clearEncoders();
  maxSpeed = keepInRange(maxSpeed, 0, 100);
  ...
  while (((inches > 0 && error > 0) || (inches < 0 && error < 0)) && !wallDetected()) {
 ...
 /*
// Creates trapezoidal motion profile
// Use negative distance to go backwards
void forwardInchesSlow(double inches, int maxSpeed)
{
  clearEncoders();
  maxSpeed = keepInRange(maxSpeed, 0, 100);

  const double minSpeed = 4;      // Lowest speed the motors will go; Turning is more precise when lower.
  const double accelRate = 20;    // Speed multiplier while accelerating (slope at start)
  const double deaccelRate = 2.75;  // Speed multiplier while deaccelerating (slope at end)

  double targetDistance = inchesToTicks(inches);  // How far the robot should travel
  double targetRange = 6;                         // Distance from the desired distance the robot has to be to stop.
  double error = targetDistance;                  // Distance from the desired distance
  double speed;                                   // Actual speed value of the motors
...
void forwardInchesFast(double inches, int maxSpeed, int msTimeout = -1)
{
  clearEncoders();
  maxSpeed = keepInRange(maxSpeed, 0, 100);
...
// Doesn't reset encoder values, but continue remembering them.
// If a movement needs to be fast to sacrifice precision for speed, this can be used afterwards
// to correct and make the ending position consistent.
void forwardInchesContinued(double inches, int maxSpeed)
{
  //clearEncoders();`
  maxSpeed = keepInRange(maxSpeed, 0, 100);

  //MinSpeed : replacement for I in PID. Ensures the movement overcomes friction.
  const double minSpeed = 5;      // Lowest speed the motors will go; Turning is more precise but jankier when lower.
...
// This honkin behemoth goes forward and turns to a certain angle at the same time
...
void fullRowTest()
{
  ...
  clearEncoders();
  task t1(dropMogoDelay); // drop the mobile goal after some encoder value is reached
  forwardInchesFast(65, 100); // go across home row

  //forkliftUp();
  gyroTurn(180, 34);
  forwardInchesFast(-26, 40, 1500); // faster
  forwardInches(2, 22);
  gyroTurn(92, 32);
...
void greenBaySkills2()
{
 ...
 // gyroTurn(155,  24);
  ...
  // back into blue, push center yellow across
  // put blue on plat
  // red back, yellow front, all to red plat
}
...
void fairgroundsSkills2()
{
  ...
  clampDown();
  task::sleep(100);
  gyroTurn(-65, 33); // turn towards blue platform
  ...
  task::sleep(50);
  ARM.startRotateTo(150, deg, 60, velocityUnits::pct);
  forwardInches(-29, 32); // back into mogo against wall -29
  forkliftDown();
  task::sleep(100);
  conveyorOn();
...
void setonSkills()
{
  ...
  task::sleep(50);
  forwardInches(-3, 25);
  gyroTurn(-476, 31);// Towards platform,  Used to be -470  -360-112  DO LEFT!!!!!! maybe -478
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    float sensitivity = 1;                                  //   (100.0 / 127.0);
    leftDrive(controllerPrim.Axis3.value() * sensitivity);    // * (100.0 / 127.0)/);
    rightDrive(controllerPrim.Axis2.value() * sensitivity);   // * (100.0 / 127.0)/);
...
int main() {
  ...
  // fairgroundsSkills
  Competition.autonomous(leftSideFake);
